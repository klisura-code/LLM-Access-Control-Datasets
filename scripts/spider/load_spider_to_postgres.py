...
# SPIDER_DB_PATH = "/home/dorde/Desktop/Access-Control-Project/spider/database"

# def map_sqlite_type_to_postgres(sqlite_type, col_name=None):
#     sqlite_type = (sqlite_type or "").upper()
#     col_name = col_name.lower() if col_name else ""

#     # Force known tricky columns to TEXT
#     if any(kw in col_name for kw in ['zip', 'event', 'type', 'status', 'category']):
#         return "TEXT"
...
# def migrate_sqlite_to_postgres(sqlite_path, schema_name, pg_conn):
...
#     # Create schema
#     pg_cursor.execute(sql.SQL("CREATE SCHEMA IF NOT EXISTS {}").format(sql.Identifier(schema_name)))
...
#         ))
#         pg_cursor.execute(sql.SQL("CREATE TABLE {}.{} ({})").format(
#             sql.Identifier(schema_name), sql.Identifier(table), sql.SQL(col_defs)
#         ))
...
#                     # Detect target PG type
#                     target_pg_type = col_defs_list[idx].split()[-1].upper()

#                     if val == '' and target_pg_type not in ['TEXT', '"TEXT"', 'VARCHAR', '"VARCHAR"']:
#                         val = None

#                     # Convert YYYYMMDD int → date
#                     if target_pg_type in ['TIMESTAMP', 'DATE'] and isinstance(val, int):
#                         try:
#                             year = val // 10000
#                             month = (val % 10000) // 100
...
#         schema_name = db_folder.lower()
#         migrate_sqlite_to_postgres(db_file, schema_name, pg_conn)

#     pg_conn.close()
#     print("All Spider databases loaded into PostgreSQL with proper types!")
